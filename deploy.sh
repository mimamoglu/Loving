...
# 5. (Optional) Set up nginx as reverse proxy if available
# =============================================================
...
# Bind gunicorn directly to port 80 (no nginx needed)
...
ExecStart=/usr/local/bin/gunicorn --workers 2 --bind 0.0.0.0:$APP_PORT --timeout 300 app:app
Restart=always
RestartSec=5
